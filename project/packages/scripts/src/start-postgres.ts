...
export async function startPostgres(
  opts: StartPostgresOptions = {},
): Promise<StartPostgresResult> {
  ...
  try {
    ...
    if (/port is already allocated|address already in use/i.test(msg)) {
      ...
      (e as { code?: string }).code = "PORT_CONFLICT";
      throw e;
    }
    throw err;
  }
...
async function findContainerByName(docker: import("dockerode"), name: string) {
  const list = await docker.listContainers({ all: true, filters: { name: [name] } });
  if (list.length === 0) return null;
  return docker.getContainer(list[0].Id);
}

async function ensureImage(docker: import("dockerode"), image: string): Promise<void> {
  const existing = await docker.listImages({ filters: { reference: [image] } });
  if (existing.length > 0) return;
  log("pulling image %s", image);
  const stream = await docker.pull(image);
  await new Promise<void>((resolve, reject) => {
    docker.modem.followProgress(stream as NodeJS.ReadableStream, (err) =>
      err ? reject(err) : resolve(),
    );
  });
}

async function waitForAccept(port: number, waitSeconds: number): Promise<void> {
  ...
  const e = new Error(
    `Postgres did not accept connections within ${waitSeconds}s: ${lastError?.message ?? "unknown"}`,
  );
  (e as { code?: string }).code = "WAIT_TIMEOUT";
  throw e;
}
...
export async function runStartPostgres(argv: string[]): Promise<number> {
  let opts: CliOptions | { help: true } | { version: true };
  try {
    opts = parseCli(argv);
  } catch (err) {
    process.stderr.write(`atw-start-postgres: ${(err as Error).message}\n`);
    return 3;
  }
  if ("help" in opts) {
  ...
  try {
    ...
    if (opts.json) {
      ...
      process.stdout.write(
        `Postgres ready on 127.0.0.1:${result.port} (${result.started ? "started" : "already running"})\n`,
      );
    }
    return 0;
  } catch (err) {
    ...
    if (code === "DOCKER_UNREACHABLE") {
      process.stderr.write(
        "atw-start-postgres: Docker daemon unreachable. Start Docker Desktop (macOS/Windows) or `systemctl start docker` (Linux).\n",
      );
      return 3;
    }
    if (code === "PORT_CONFLICT") {
      process.stderr.write(
        `atw-start-postgres: ${msg}. Retry with --port <free> or stop the occupant.\n`,
      );
      return 4;
    }
