...
export function detectAdminOperations(
...
function classifyOperation(
  ...
  rawPaths: Map<string, Record<string, unknown>>,
): AdminFlag | null {
...
function extractRawPaths(raw: unknown): Map<string, Record<string, unknown>> {
  const out = new Map<string, Record<string, unknown>>();
  if (!raw || typeof raw !== "object") return out;
  const paths = (raw as { paths?: Record<string, unknown> }).paths;
  if (!paths) return out;
  for (const [p, item] of Object.entries(paths)) {
    if (item && typeof item === "object") out.set(p, item as Record<string, unknown>);
  }
  return out;
}

function lookupRawOperation(
  rawPaths: Map<string, Record<string, unknown>>,
  path: string,
  method: string,
...
function hasAdminVendorExtension(op: Record<string, unknown>): boolean {
  for (const key of Object.keys(op)) {
    if (!key.startsWith("x-")) continue;
    if (key.toLowerCase().startsWith("x-admin")) {
      const val = op[key];
      if (val === true || (typeof val === "string" && val.length > 0)) return true;
    }
  }
  return false;
}
