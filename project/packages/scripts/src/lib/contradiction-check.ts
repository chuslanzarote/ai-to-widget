 ...
 */
export function detectContradictions(answers: ContradictionAnswers): ContradictionReport {
...
function sharesActionVerb(a: string, b: string): boolean {
...
function significantOverlap(a: string, b: string): string[] {
  const setB = new Set(tokens(b));
  const out = new Set<string>();
  for (const t of tokens(a)) {
    if (setB.has(t)) out.add(t);
  }
  return [...out];
}
...
function buildPrompt(contradictions: Contradiction[]): string {
...
function describeKind(kind: Contradiction["kind"]): string {
  switch (kind) {
