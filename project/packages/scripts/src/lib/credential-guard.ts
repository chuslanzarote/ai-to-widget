...
export interface CredentialFindings {
  found: boolean;
  matches: { kind: "scheme" | "kv-pair"; sample: string }[];
}

export function detectCredentials(input: string): CredentialFindings {
  const matches: CredentialFindings["matches"] = [];

  for (const rx of CONN_STRING_SCHEMES) {
    const m = input.match(rx);
    if (m) matches.push({ kind: "scheme", sample: redact(m[0]) });
  }
...
export const REFUSAL_MESSAGE =
  "I don't accept database connection strings or credentials. Please export " +
