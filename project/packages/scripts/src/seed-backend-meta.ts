...
const META_FILES: readonly string[] = [
  "Dockerfile",
  ".dockerignore",
  "package.json",
  "tsconfig.json",
...
export function defaultBackendPackageDir(): string {
  const here = fileURLToPath(import.meta.url);
  const pkgScripts = path.resolve(path.dirname(here), "..");
  return path.resolve(pkgScripts, "..", "backend");
}

export async function seedBackendMeta(
  opts: SeedBackendMetaOptions,
): Promise<SeededFile[]> {
...
async function inlineTsconfigBase(raw: string, srcDir: string): Promise<string> {
  let parsed: {
    extends?: string;
  ...
  try {
    base = JSON.parse(baseRaw) as typeof base;
  } catch {
    return raw;
  }
  const merged = {
