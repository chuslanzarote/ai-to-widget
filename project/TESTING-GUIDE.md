...
Paste each `@sha256:…` into `tools/dev/docker-compose.yml`, replacing
the three `# TODO(compose-digest): pin @sha256:<digest>` markers.
...
git add tools/dev/docker-compose.yml       # pinned digests
