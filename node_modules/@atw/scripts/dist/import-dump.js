 ...
 */
export function sanitizePgDump17(sql) {
    const out = [];
    for (const line of sql.split(/\r?\n/)) {
        const trimmed = line.trim();
        if (/^SET\s+transaction_timeout\b/i.test(trimmed))
            continue;
        if (/^\\(restrict|unrestrict)\b/.test(trimmed))
            continue;
        out.push(line);
    }
    let joined = out.join("\n");
    joined = joined.replace(/^[ \t]*ALTER\s+TABLE\s+(?:ONLY\s+)?[^\n;]*\bOWNER\s+TO\s+[^;]*;[ \t]*\r?\n?/gim, "");
    return joined;
}
...
export function filterDump(sql, map, targetSchema = "client_ref") {
    ...
    for (const st of statements) {
        ...
        const rewrite = (text) => text.replace(/\b(?:public\.)?("?)(\w+)\1/g, (match, _q, _name) => match);
            ...
            if (dropFor && dropFor.size > 0) {
                ...
                if (colList) {
                    ...
                    if (keepMask.some((k) => !k)) {
                        ...
                        const newHead = head.replace(/\(([^)]*)\)\s*$/, `(${keptCols.map((c) => `"${c}"`).join(", ")}) `);
                        const newTail = tail.replace(/\(([^)]*)\)/, (_m, body) => {
...
async function loadSchemaMapForImport(schemaMapPath) {
    ...
    for (const ent of art.content.entities) {
        if (ent.classification === "indexable" || ent.classification === "reference") {
            for (const t of ent.sourceTables)
                includedTables.add(t.toLowerCase());
 ...
 */
export async function importDump(opts) {
...
export async function runImportDump(argv) {
    let opts;
    try {
        opts = parseCli(argv);
    }
    catch (err) {
        process.stderr.write(`atw-import-dump: ${err.message}\n`);
        return 3;
    }
    if ("help" in opts) {
        process.stdout.write("atw-import-dump --dump <path> --schema-map <path> [--replace] [--json]\n");
        return 0;
    }
    if ("version" in opts) {
    ...
    try {
        ...
        if (opts.json) {
            ...
            process.stdout.write(`imported=${result.imported.length} excluded_pii_tables=${result.excluded_pii_tables.length} dropped_pii_columns=${result.dropped_pii_columns.length}\n`);
        }
        return 0;
    }
    catch (err) {
