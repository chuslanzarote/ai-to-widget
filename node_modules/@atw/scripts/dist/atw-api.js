...
import { parseOpenAPI, Swagger20DetectedError, OpenAPIFetchError, ParseOpenAPIError, DuplicateOperationIdError, } from "./parse-openapi.js";
import { DEFAULT_INPUT_HASHES_PATH, readInputHashes, writeInputHashes, } from "./lib/input-hashes.js";
import { loadExistingProject } from "./init-project.js";
import { emitHostRequirements, HOST_REQUIREMENTS_REL, } from "./host-requirements.js";
 ...
 */
export function canonicaliseOpenAPI(doc) {
    return JSON.stringify(sortKeysDeeply(doc), null, 2) + "\n";
}
function sortKeysDeeply(v) {
    if (Array.isArray(v))
        return v.map(sortKeysDeeply);
    if (v !== null && typeof v === "object") {
        const src = v;
        const sortedKeys = Object.keys(src).sort((a, b) => a < b ? -1 : a > b ? 1 : 0);
        const out = {};
        for (const k of sortedKeys)
            out[k] = sortKeysDeeply(src[k]);
        return out;
    }
    return v;
}
 ...
 */
export async function runAtwApi(opts) {
    ...
    const hash = "sha256:" + createHash("sha256").update(canonicalBytes).digest("hex");
...
export async function runAtwApiCli(argv) {
    let opts;
    try {
        opts = parseCli(argv);
    }
    ...
    if ("help" in opts) {
        process.stdout.write("atw-api --source <path|url> [--backup] [--json]\n");
        return 0;
    }
    if ("version" in opts) {
    ...
    try {
        ...
        if (opts.json) {
            ...
            if (result.hostRequirements &&
                result.hostRequirements.action !== "skipped") {
                process.stdout.write(`${result.hostRequirements.action} ${result.hostRequirements.path}\n`);
...
//# sourceMappingURL=atw-api.js.map