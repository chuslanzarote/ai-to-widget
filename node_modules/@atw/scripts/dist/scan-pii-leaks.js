...
export function findMatches(normalizedDoc, originalDoc, values) {
    const hits = [];
    for (const v of values) {
        const needle = normalizeForMatch(v.value);
        if (needle.length < 3)
            continue; // avoid pathological short matches
...
export async function scanPiiLeaks(opts) {
    const piiCols = [];
        ...
        for (const c of ent.columns) {
            if (c.decision === "exclude-pii") {
                for (const t of ent.sourceTables)
                    piiCols.push({ table: t, column: c.name });
            }
        }
    }
    for (const ex of opts.schemaMap.piiExcluded) {
        for (const col of ex.columns)
            piiCols.push({ table: ex.table, column: col });
    }
    ...
    try {
        ...
        for (const { table, column } of piiCols) {
            try {
                const res = await client.query(`SELECT CAST("${column}" AS text) AS v FROM client_ref."${table}" WHERE "${column}" IS NOT NULL`);
                ...
                log("skip %s.%s: %s", table, column, err.message);
            }
        }
        // Scan every atw_documents row.
        const docs = await client.query(`SELECT entity_type, entity_id, document, facts, categories FROM atw_documents`);
...
export async function runScanPiiLeaks(argv) {
    let opts;
    try {
        opts = parseCli(argv);
    }
    catch (err) {
        process.stderr.write(`atw-scan-pii-leaks: ${err.message}\n`);
        return 3;
    }
    if ("help" in opts) {
    ...
    try {
        ...
        if (opts.json) {
            ...
            process.stdout.write(`clean=${result.clean} values_checked=${result.values_checked} matches=${result.matches.length}\n`);
        }
...
//# sourceMappingURL=scan-pii-leaks.js.map