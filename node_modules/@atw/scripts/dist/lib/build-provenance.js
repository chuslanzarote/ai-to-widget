...
import { BuildProvenanceEntrySchema, } from "./schemas/build-provenance.js";
export const DEFAULT_PROVENANCE_PATH = ".atw/artifacts/build-provenance.json";
export function provenancePath(projectRoot) {
 ...
 */
export async function appendProvenance(projectRoot, entry) {
 ...
 * search `success`).
 */
export function findCachedSuccess(projectRoot, phase, inputHashes, modelSnapshot) {
    const file = readProvenance(projectRoot);
    for (let i = file.entries.length - 1; i >= 0; i--) {
        ...
        if (e.status !== "success")
            continue;
        if (modelSnapshot && e.model_snapshot !== modelSnapshot)
            continue;
        if (!hashesEqual(e.input_hashes ?? {}, inputHashes))
            continue;
        return e;
    }
    return null;
...
export function startPhase(phase, buildId) {
    ...
    return {
        ...
        async finish(projectRoot, overrides) {
            const entry = BuildProvenanceEntrySchema.parse({
                build_id: buildId,
                phase,
 ...
 */
export function summarize(entries, buildId) {
    const own = entries.filter((e) => e.build_id === buildId);
    if (own.length === 0) {
        return "✅ Build complete.";
    }
    const issues = own.filter((e) => e.status !== "success" && e.status !== "success_cached");
    if (issues.length === 0) {
        return "✅ Build complete.";
    }
    const lines = [
    ...
    for (const e of issues) {
        const reason = e.status === "failed"
            ? e.failed_reason
            : e.status === "skipped"
                ? e.skipped_reason
                : (e.warnings ?? []).join("; ") || null;
        const hint = e.next_hint ? ` · hint: ${e.next_hint}` : "";
        lines.push(`  [${e.phase}] ${e.status}${reason ? ` — ${reason}` : ""}${hint}`);
    }
    return lines.join("\n");
}
