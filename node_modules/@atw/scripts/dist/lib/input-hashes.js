...
export function computeInputHashes(projectRoot, sqlDumpRelPath, promptTemplateVersion) {
    const files = {};
    ...
    if (sqlDumpRelPath) {
        ...
        if (existsSync(abs))
            files[sqlDumpRelPath] = sha256File(abs);
    }
    return {
        schema_version: "1",
        files,
...
export function readInputHashes(projectRoot) {
    ...
    try {
        ...
        if (!parsed.files || typeof parsed.files !== "object")
            return null;
        if (typeof parsed.prompt_template_version !== "string")
            return null;
        return parsed;
    }
    catch {
...
export async function writeInputHashes(projectRoot, hashes) {
    const abs = join(projectRoot, DEFAULT_INPUT_HASHES_PATH);
 ...
 */
export function diffInputHashes(prior, current) {
    if (!prior) {
        return {
    ...
    for (const k of all) {
        if (prior.files[k] !== current.files[k])
            changed.push(k);
    }
    const promptVersionChanged = prior.prompt_template_version !== current.prompt_template_version;
    return {
        changedKeys: changed,
 ...
 */
export function computeBackendSourceTree(entries) {
...
//# sourceMappingURL=input-hashes.js.map