 ...
 */
export async function assembleEntityInput(opts: AssembleOptions): Promise<AssembledEntityInput> {
  const ent = opts.schemaMap.entities.find((e) => e.name === opts.entityType);
  if (!ent) {
    const e = new Error(
      `entity_type "${opts.entityType}" not found in schema-map`,
    );
  ...
  try {
    ...
    if (indexedCols.length === 0) {
      const e = new Error(
        `entity "${opts.entityType}" has no indexed/reference columns — nothing to send to Opus`,
      );
      (e as { code?: string }).code = "SCHEMA_CORRUPT";
      throw e;
    }
    ...
    try {
      res = await client.query(
        `SELECT ${cols} FROM client_ref."${primaryTable}" WHERE id = $1 LIMIT 1`,
        [opts.entityId],
      );
    } catch (err) {
      ...
      if (/relation .* does not exist/i.test(msg)) {
        const e = new Error(
          `Schema-map references missing table "${primaryTable}" in client_ref`,
        );
        (e as { code?: string }).code = "SCHEMA_CORRUPT";
        throw e;
      }
    ...
    if (res.rowCount === 0) {
      const e = new Error(
        `entity_id "${opts.entityId}" not found in client_ref."${primaryTable}"`,
      );
      (e as { code?: string }).code = "ENTITY_NOT_FOUND";
      throw e;
    }
    const primary = res.rows[0] as Record<string, unknown>;
    ...
    for (const ref of ent.joinedReferences) {
      try {
        ...
        related.push({
          relation: ref,
          rows: refRes.rows as Record<string, unknown>[],
        });
      } catch (err) {
        log("related query failed for %s: %s", ref, (err as Error).message);
      }
    }

    const out: AssembledEntityInput = {
...
function parseCli(argv: string[]): CliOptions | { help: true } | { version: true } {
  ...
  if (!values["entity-type"]) throw new Error("--entity-type <t> is required");
  if (!values["entity-id"]) throw new Error("--entity-id <id> is required");
  if (!values["schema-map"]) throw new Error("--schema-map <path> is required");
  if (!values.brief) throw new Error("--brief <path> is required");
...
export async function runAssembleEntityInput(argv: string[]): Promise<number> {
  let opts: CliOptions | { help: true } | { version: true };
  try {
    opts = parseCli(argv);
  } catch (err) {
    process.stderr.write(`atw-assemble-entity-input: ${(err as Error).message}\n`);
    return 3;
  }
  if ("help" in opts) {
    process.stdout.write(
      "atw-assemble-entity-input --entity-type <t> --entity-id <id> --schema-map <path> --brief <path> [--json]\n",
    );
    return 0;
  }
  if ("version" in opts) {
  ...
  try {
    ...
    const schemaArt = await loadArtifactFromFile(
      "schema-map",
      path.resolve(opts.schemaMap),
    );
