...
const PRIOR_COMMAND_FOR_ARTIFACT: Record<string, string> = {
 ...
 */
export async function runBuild(flags: OrchestratorFlags): Promise<OrchestratorResult> {
  ...
  if (flags.entitiesOnly && flags.noEnrich) {
    process.stderr.write(
      "Error: --entities-only and --no-enrich cannot be combined. Enrichment is how entities are populated.\n",
    );
  ...
  if (planSummary.estimatedCostUsd !== null) {
    process.stdout.write(
      `  Estimated cost    : $${planSummary.estimatedCostUsd.toFixed(2)} (from build-plan.md)\n`,
    );
  }
  ...
  const priorInputHashes: InputHashes | null = readInputHashes(flags.projectRoot);
  ...
  const currentInputHashesPreboot = (() => {
    try {
      ...
      log("computeInputHashes (preboot) failed: %s", (err as Error).message);
      return null;
    }
  })();
  const incrementalDiff =
    currentInputHashesPreboot !== null
  ...
  if (
    ...
    try {
      ...
      log("nothing-to-do manifest/hashes write failed: %s", (err as Error).message);
    }
    process.stdout.write(
  ...
  const skipEnrichmentForIncremental = (() => {
    ...
    if (!priorInputHashes || !incrementalDiff) return false;
    if (incrementalDiff.promptVersionChanged) return false;
    return (
      incrementalDiff.changedKeys.length === 1 &&
  ...
  if (
    ...
    process.stderr.write(
      "Warning: brief.md changed since last build. Consider /atw.build --force to re-enrich all entities.\n",
    );
  }
  ...
  if (!flags.noEnrich && !skipEnrichmentForIncremental && !flags.opusClient) {
    ...
    if (!hasKey) {
      process.stderr.write(
        "Anthropic API authentication failed. Set ANTHROPIC_API_KEY in your shell environment and re-run /atw.build.\n",
      );
      return { exitCode: 3, manifest: null };
    }
  }
  ...
  const failures: BuildManifest["failures"] = [];
  const opusTotals: TokenUsage & { calls: number } = {
    calls: 0,
    input_tokens: 0,
    output_tokens: 0,
  };
  ...
  let concurrencyControllerRef: ConcurrencyController | null = null;

  try {
    // 1. BOOT -----------------------------------------------------------
    progress.banner(
      banner("BOOT", `Starting atw_postgres (pgvector/pgvector:pg16) on :${postgresPort} ...`),
    );
    ...
    progress.banner(
      banner(
        "MIGRATE",
        `${migrated.applied.length} applied, ${migrated.skipped.length} already present`,
      ),
    );
    ...
    if (!flags.noEnrich && !skipEnrichmentForIncremental) {
      ...
      progress.banner(
        banner("ENRICH", `${entityIds.length} entities to enrich @ concurrency ${concurrency}`),
      );
          ...
          limit(async () => {
            ...
            try {
              ...
              if (concurrencyController.halt) {
                const e = new Error(
                  "Opus rate limits exhausted even at reduced concurrency",
                );
              ...
              if (callResult.validationFailedTwice) {
                ...
                failures.push({
                  entity_type: ent.type,
                  entity_id: ent.id,
                  reason: "validation_failed_twice" as ManifestFailureReason,
              ...
              if ("insufficient_data" in callResult.response) {
                failures.push({
                  entity_type: ent.type,
                  entity_id: ent.id,
                  reason: "insufficient_data" as ManifestFailureReason,
              ...
              const reason: ManifestFailureReason = classifyFailure(err);
              failures.push({
                entity_type: ent.type,
                entity_id: ent.id,
                reason,
              ...
              progress.report({
                ...
                eta_seconds: etaFromRate(
                  processed,
                  entityIds.length,
                  (Date.now() - enrichStart) / 1000,
                ),
              });
            }
          }),
    ...
    if (!flags.noEnrich && !abortState.aborted) {
      ...
      if (!scan.clean) {
        process.stderr.write(
          `Compliance scan failed: ${scan.matches.length} PII value(s) leaked. See build-manifest.json.\n`,
        );
      }
    }
    ...
    // 10. MANIFEST ------------------------------------------------------
    const completedAt = new Date();
    const result: ManifestResult = abortState.aborted
      ? "aborted"
      ...
      opus: {
        ...
        ...(planSummary.estimatedCostUsd !== null
          ? {
              ...
              cost_variance_pct: computeCostVariancePct(
                actualCostUsd,
                planSummary.estimatedCostUsd,
              ),
            }
          : {}),
      },
    ...
    if (result === "success" || result === "nothing-to-do") {
      try {
        ...
        await writeInputHashes(
          flags.projectRoot,
          computeInputHashes(flags.projectRoot, sqlDumpRel, PROMPT_TEMPLATE_VERSION),
        );
      } catch (err) {
        log("failed to write input-hashes.json: %s", (err as Error).message);
      }
    }
    ...
    const costLine = (() => {
      ...
      if (planSummary.estimatedCostUsd === null) return actualStr;
...
function banner(
  phase:
    ...
    | "ABORT",
  message: string,
): import("./lib/types.js").PipelineProgress {
  return {
    phase,
    processed: 0,
    total: 0,
...
function etaFromRate(processed: number, total: number, elapsedSec: number): number | null {
...
function classifyFailure(err: unknown): ManifestFailureReason {
  ...
  if (code === "OPUS_400") return "opus_400";
  if (code === "OPUS_5XX_TWICE") return "opus_5xx_twice";
  if (code === "VALIDATION_FAILED") return "validation_failed_twice";
  const msg = err instanceof Error ? err.message : String(err);
  if (/validation/i.test(msg)) return "validation_failed_twice";
  if (/missing|not found/i.test(msg)) return "missing_source_data";
  if (/400|bad request/i.test(msg)) return "opus_400";
  if (/5\d\d|server error/i.test(msg)) return "opus_5xx_twice";
  return "opus_400";
}
...
function schemaArtifactToImport(sm: SchemaMapArtifact): SchemaMapForImport {
  ...
  for (const ent of sm.entities) {
    if (ent.classification === "indexable" || ent.classification === "reference") {
      for (const t of ent.sourceTables) included.add(t.toLowerCase());
...
/**
 * T070 / US4 — read `.atw/artifacts/build-plan.md` and extract the cost
 * estimate and duration range (if present) for the plan-summary banner
 * and for manifest.opus.estimated_cost_usd. Returns {null, null} on any
 * parse failure so the rest of the build still runs.
 */
interface PlanSummary {
  estimatedCostUsd: number | null;
  estimatedDurationRange: string | null;
}

async function readPlanSummary(projectRoot: string): Promise<PlanSummary> {
  const path = join(projectRoot, ".atw", "artifacts", "build-plan.md");
  try {
    const art = await loadArtifactFromFile("build-plan", resolve(path));
    if (art.kind !== "build-plan") return { estimatedCostUsd: null, estimatedDurationRange: null };
    ...
    log("readPlanSummary failed: %s", (err as Error).message);
    return { estimatedCostUsd: null, estimatedDurationRange: null };
  }
}

async function loadSchemaMap(projectRoot: string): Promise<SchemaMapArtifact> {
...
async function findSqlDump(projectRoot: string): Promise<string | null> {
...
async function findSqlDumpRelative(projectRoot: string): Promise<string | null> {
...
async function existingSourceHashMatches(params: {
  ...
  try {
    ...
    try {
      const res = await client.query<{ source_hash: string }>(
        "SELECT source_hash FROM atw_documents WHERE entity_type = $1 AND entity_id = $2",
    ...
    log("source-hash pre-check failed: %s", (err as Error).message);
    return false;
  }
}
  ...
  try {
    for (const ent of params.schemaMap.entities) {
      ...
      try {
        ...
        log("listIndexable skip %s.%s: %s", primary, ent.name, (err as Error).message);
      }
    }
  } finally {
    await client.end().catch(() => void 0);
...
function hashAllInputs(projectRoot: string): Record<string, string> {
  ...
  for (const rel of REQUIRED_ARTIFACTS) {
    ...
    if (existsSync(abs)) {
      try {
        ...
        out[rel.split("/").pop() as string] = "sha256:" + h;
      } catch {
        // skip
      }
    }
...
function captureEnvironment(): BuildManifest["environment"] {
  return {
 ...
 */
export function parseArgs(argv: string[], projectRoot: string): OrchestratorFlags {
  const flags: OrchestratorFlags = { projectRoot };
