...
export async function applyMigrations(opts: ApplyOptions): Promise<ApplyResult> {
  ...
  try {
    ...
    const appliedRows = await client.query<{ filename: string; sha256: string }>(
      "SELECT filename, sha256 FROM atw_migrations ORDER BY id",
    );
    ...
    for (const m of files) {
      ...
      if (priorSha !== undefined) {
        if (priorSha !== m.sha256) {
          ...
          const e = new Error(
            `Migration ${m.filename} was modified after apply (sha mismatch)`,
          );
          (e as { code?: string }).code = "MIGRATION_EDITED";
          throw e;
        }
      ...
      try {
        ...
        await client.query(
          "INSERT INTO atw_migrations (filename, sha256) VALUES ($1, $2)",
          [m.filename, m.sha256],
        );
        ...
        failed.push(base);
        const e = new Error(`Migration ${m.filename} failed: ${(err as Error).message}`);
        (e as { code?: string }).code = "MIGRATION_FAILED";
...
export async function runApplyMigrations(argv: string[]): Promise<number> {
  let opts: CliOptions | { help: true } | { version: true };
  try {
    opts = parseCli(argv);
  } catch (err) {
    process.stderr.write(`atw-apply-migrations: ${(err as Error).message}\n`);
    return 3;
  }
  if ("help" in opts) {
    process.stdout.write(
      "atw-apply-migrations [--dry-run] [--json] [--host ...] [--port ...] [--user ...] [--password ...] [--database ...]\n",
    );
    return 0;
  }
  if ("version" in opts) {
