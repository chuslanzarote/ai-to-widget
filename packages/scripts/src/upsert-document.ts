...
export async function upsertDocument(opts: UpsertOptions): Promise<UpsertResult> {
  if (opts.row.embedding.length !== EMBEDDING_DIM) {
    const e = new Error(
      `Embedding dimension mismatch: got ${opts.row.embedding.length}, expected ${EMBEDDING_DIM}`,
    );
  ...
  try {
    ...
    const e = new Error(`Postgres not reachable: ${(err as Error).message}`);
    (e as { code?: string }).code = "PG_UNREACHABLE";
    throw e;
  }
  try {
    const existing = await client.query<{ source_hash: string }>(
      ...
      [opts.row.entity_type, opts.row.entity_id],
    );
...
async function readRow(p?: string): Promise<unknown> {
  if (p) {
...
export async function runUpsertDocument(argv: string[]): Promise<number> {
  let opts: CliOptions | { help: true } | { version: true };
  try {
    opts = parseCli(argv);
  } catch (err) {
    process.stderr.write(`atw-upsert-document: ${(err as Error).message}\n`);
    return 3;
  }
  if ("help" in opts) {
