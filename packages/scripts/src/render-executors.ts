...
export class InvalidSubstitutionError extends Error {
  readonly code = "INVALID_SUBSTITUTION" as const;
  readonly tool: string;
...
export async function renderExecutors(
  ...
  if (!parsed.success) {
    throw new Error(
      `renderExecutors: post-build catalog failed schema validation: ${parsed.error.issues[0]?.message ?? "unknown"}`,
    );
  }
  ...
  let action: "created" | "unchanged" | "rewritten";
  if (prior === null) {
    action = "created";
...
function manifestEntryToExecutor(
  entry: ActionManifestEntry,
  opts: RenderExecutorsOptions,
  warnings: string[],
): ActionExecutorEntry {
  ...
  const pathBucket: Record<string, string> = {};
  for (const ph of placeholders) {
    if (!(ph in props)) {
      throw new InvalidSubstitutionError(
        entry.toolName,
        ph,
        `manifest entry "${entry.toolName}" references path placeholder "{${ph}}" but has no matching parameter in properties`,
      );
    }
  ...
  const bodyBucket: Record<string, string> = {};
  const queryBucket: Record<string, string> = {};
  if (entry.source.method === "GET") {
    for (const n of nonPathNames) queryBucket[n] = `arguments.${n}`;
  } else {
    for (const n of nonPathNames) bodyBucket[n] = `arguments.${n}`;
  }

  const headers: Record<string, string> =
    entry.source.method === "GET"
      ? {}
  ...
  try {
    ...
    warnings.push(
      `could not resolve host origin for action "${entry.toolName}" (hostOrigin=${opts.hostOrigin})`,
    );
  }
  ...
  if (!parsed.success) {
    throw new Error(
      `renderExecutors: entry "${entry.toolName}" failed schema validation: ${parsed.error.issues[0]?.message ?? "unknown"}`,
    );
  }
  return parsed.data;
}
...
function defaultSuccessStatuses(method: string): number[] {
  switch (method) {
    case "POST":
      return [200, 201];
    case "DELETE":
...
export function canonicaliseCatalog(catalog: ActionExecutorsCatalog): string {
