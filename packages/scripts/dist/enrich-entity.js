...
import { AssembledEntityInputSchema, EnrichmentResponseSchema, } from "./lib/types.js";
import { computeCostUsd, OPUS_PRICING } from "./lib/pricing.js";
import { validateEnrichment, } from "./lib/enrichment-validator.js";
 ...
 */
export async function enrichEntity(opts) {
    ...
    if (!anchorValidation) {
        ...
        const raw = await callWithHttpRetries(() => client.createMessage({ model, system, user }), { onHttpStatus: opts.onHttpStatus });
        const parsed = safeParseJson(raw.contentText);
        if (!parsed.ok) {
            const e = new Error(`Opus returned non-JSON content: ${parsed.message}`);
            e.code = "OPUS_INVALID_JSON";
            throw e;
        }
        const zodRes = EnrichmentResponseSchema.safeParse(parsed.value);
        if (!zodRes.success) {
            const e = new Error(`Opus response failed schema validation: ${zodRes.error.issues[0]?.message ?? "unknown"}`);
    ...
    log("validator rejected first response for %s/%s: %s", opts.input.entity_type, opts.input.entity_id, firstCheck.rule);
 ...
 */
async function callAndParseRaw(client, model, system, user, onHttpStatus) {
    ...
    const raw = await callWithHttpRetries(() => client.createMessage({ model, system, user }), { onHttpStatus });
    const parsed = safeParseJson(raw.contentText);
    if (!parsed.ok) {
        const e = new Error(`Opus returned non-JSON content: ${parsed.message}`);
        e.code = "OPUS_INVALID_JSON";
 ...
 */
export function buildSharpeningUser(originalUser, rule, detail, input) {
    const lines = [
...
export async function defaultOpusClient(_model, apiKey) {
    ...
    const anthropic = new sdk({ apiKey: key });
    return {
        async createMessage(args) {
            return callWithHttpRetries(() => anthropic.messages.create({
                model: args.model,
                max_tokens: 4096,
                system: args.system,
 ...
 * backoff only.
 */
export async function callWithHttpRetries(fn, opts) {
    ...
    while (true) {
        try {
            ...
            if (typeof status === "number")
                onHttpStatus?.(status);
            if (status === 400) {
                const e = new Error(`Opus returned HTTP 400: ${err.message ?? "bad request"}`);
                e.code = "OPUS_400";
                throw e;
            }
...
export async function runEnrichEntity(argv) {
    let opts;
    try {
        opts = parseCli(argv);
    }
    catch (err) {
        process.stderr.write(`atw-enrich-entity: ${err.message}\n`);
        return 3;
    }
    if ("help" in opts) {
        process.stdout.write("atw-enrich-entity --input <path-or-stdin> --build-plan <path> [--retry-strategy aggressive|conservative] [--json]\n");
        return 0;
    }
    if ("version" in opts) {
    ...
    try {
        ...
        if (opts.json) {
            ...
            process.stdout.write(`kind=${"kind" in result.response ? result.response.kind : "insufficient_data"} cost=$${result.costUsd.toFixed(4)}\n`);
        }
        return 0;
    }
    catch (err) {
...
//# sourceMappingURL=enrich-entity.js.map