...
import { ActionManifestSchema, actionManifestJsonSchema, checkManifestInvariants, } from "./lib/schemas/action-manifest.js";
...
import { computeCostUsd, MODEL_PRICING } from "./lib/pricing.js";
import { estimateLlmCallCost, formatPreCallAnnouncement, preCallCountdown, } from "./lib/cost-estimator.js";
export class ClassifyValidationError extends Error {
    issues;
    constructor(issues) {
        super(`LLM-emitted manifest failed validation:\n` +
...
function buildUserMessage(projectMd, briefMd, openapiJson, outputSchemaJson) {
    return [
        "<project_md>",
        projectMd,
        "</project_md>",
...
export async function classifyActions(opts) {
    const projectRoot = opts.projectRoot ?? process.cwd();
    const cfg = loadProjectConfig({ projectRoot });
    const modelSnapshot = cfg.model_snapshot;
    ...
    process.stdout.write(`${formatPreCallAnnouncement({
        phase: "CLASSIFY",
        operationCount: parsed.operations.length,
        modelSnapshot,
        estimate,
    ...
    const retryResult = await withLLMRetry(async () => {
        ...
        return await anthropic.messages.create({
            ...
            tools: [
                {
                    name: "emit_manifest",
                    description: "Emit the validated action manifest for the source OpenAPI document.",
    ...
    const sourceWriteBodies = new Map();
    ...
    const manifestPath = opts.outPath ?? path.join(projectRoot, ".atw/artifacts/action-manifest.md");
    writeActionManifest(manifestPath, finalManifest);
    const usage = message.usage;
    const actualCostUsd = computeCostUsd({ input_tokens: usage.input_tokens, output_tokens: usage.output_tokens }, modelSnapshot);
    const variancePct = estimatedCostUsd === 0
        ? 0
...
/* ================================================================== CLI === */
export async function runClassifyActions(argv) {
    let source = null;
    let outPath = null;
    ...
    try {
        ...
        process.stdout.write(`[classify] Manifest validated: ${result.manifest.operations.length} operations in scope.\n` +
        ...
        if (err instanceof LLMRetryError) {
            process.stderr.write(`atw-classify-actions: ${err.message} (attempts: ${err.attempts.length})\n`);
            return 4;
        }
        ...
        if (e.issues)
            process.stderr.write(`${JSON.stringify(e.issues, null, 2)}\n`);
        return 1;
    }
}
...
//# sourceMappingURL=classify-actions.js.map