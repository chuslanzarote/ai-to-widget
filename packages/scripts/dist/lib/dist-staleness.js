...
function walkTs(dir, root, out) {
    ...
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        if (SKIP_FRAGMENTS.some((s) => rel.includes(s)))
            continue;
        out.push(abs);
    }
}
export function checkDistStaleness(opts) {
 ...
 */
export function enforceDistFreshness(opts) {
    if (process.env.ATW_SKIP_DIST_CHECK === "1")
        return;
    ...
    process.stderr.write(`[atw] dist/ is stale: ${list}${more} — source newer than (or missing) compiled output.\n` +
        `[atw] Run \`npm run build\` and try again.\n`);
    process.exit(2);
}
//# sourceMappingURL=dist-staleness.js.map