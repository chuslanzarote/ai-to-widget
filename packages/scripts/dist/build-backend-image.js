 ...
 */
export async function buildBackendImage(opts) {
    ...
    await new Promise((resolve, reject) => {
        docker.modem.followProgress(stream, (err) => (err ? reject(err) : resolve()), (evt) => {
            if (evt.error)
                lastError = evt.error;
            if (evt.errorDetail?.message)
                lastError = evt.errorDetail.message;
            if (evt.stream)
                log(evt.stream.trimEnd());
        });
    });
    if (lastError) {
...
async function assertNoSecretsInContext(dir) {
    const offenders = [];
    const walk = async (d, rel) => {
        ...
        for (const item of items) {
            ...
            if (/^\.env(\..+)?$/.test(item.name))
                offenders.push(relp);
            if (/\.(pem|p12|pfx|key)$/.test(item.name))
                offenders.push(relp);
            if (item.name === "id_rsa" || item.name === "id_ed25519")
                offenders.push(relp);
        }
    };
    await walk(dir, "");
    if (offenders.length > 0) {
        const e = new Error(`refusing to build: secret-shaped files in build context: ${offenders.join(", ")}`);
        e.code = "SECRET_IN_CONTEXT";
        throw e;
    }
}
async function collectBuildContext(dir) {
    const tar = await import("tar-fs").catch(() => null);
    if (!tar) {
        throw new Error("tar-fs is required to build docker images. Install it as a dependency of @atw/scripts.");
    }
    const ignore = (name) => {
...
export async function runBuildBackendImage(argv) {
    let opts;
    try {
        opts = parseCli(argv);
    }
    catch (err) {
        process.stderr.write(`atw-build-backend-image: ${err.message}\n`);
        return 3;
    }
    if ("help" in opts) {
        process.stdout.write("atw-build-backend-image [--context-dir <p>] [--dockerfile <p>] [--tag <ref>] [--json]\n");
        return 0;
    }
    if ("version" in opts) {
    ...
    try {
        ...
        if (code === "DOCKER_UNREACHABLE") {
            process.stderr.write("atw-build-backend-image: Docker daemon unreachable. Start Docker Desktop (macOS/Windows) or `systemctl start docker` (Linux).\n");
            return 3;
        }
...
//# sourceMappingURL=build-backend-image.js.map