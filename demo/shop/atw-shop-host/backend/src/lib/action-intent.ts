...
export function buildActionIntent(
  ...
  const resolvedPath = input.tool.http.path.replace(/\{([a-zA-Z0-9_]+)\}/g, (_, key) => {
...
function renderDescription(
  ...
  if (tool.description_template) {
    return tool.description_template.replace(/\{([a-zA-Z0-9_]+)\}/g, (_, key) => {
      const v = args[key];
...
function renderSummary(
  ...
  if (!tool.summary_fields || tool.summary_fields.length === 0) return undefined;
  const out: Record<string, string> = {};
  for (const field of tool.summary_fields) {
    const v = args[field];
    if (v !== undefined && v !== null) out[field] = String(v);
  }
